...
MODEL="agent-ai/agent-1.0-7b-instruct"

python vllm_inference.py --model_name_or_path $MODEL --saved_path output/20240121-Jul.agent-1.0-7b-instruct.jsonl
```

If you want to evaluate the model with COT, please add `--cot` to the command:
```bash
python vllm_inference.py --model_name_or_path $MODEL --saved_path output/20240121-Jul.agent-1.0-7b-instruct.jsonl --cot
```

2. Run `evaluate_leetcode.py` to get evaluation results.
```bash
python evaluate_leetcode.py --generation_path output/20240121-Jul.agent-1.0-7b-instruct.jsonl --result_path output/20240121-Jul.agent-1.0-7b-instruct.result.jsonl
```
...
| | | | |
| Agent 1.0-Instruct     | 1.3B | 22.2%     | 1.1%        | 4.5%      | 7.2%         |
| Agent 1.0-Instruct + CoT | 1.3B | 22.2%   | 2.2%        | 2.3%      | 7.2%         |
| Agent 1.0-Instruct     | 6.7B | 44.4%     | 12.1%       | 9.1%      | 19.4%        |
| Agent 1.0-Instruct + CoT | 6.7B | 44.4%   | 17.6%       | 4.5%      | 21.1%        |
| Agent 1.0-Instruct     | 33B  | 57.8%     | 22.0%       | 9.1%      | 27.8%        |
| Agent 1.0-Instruct + CoT | 33B | 53.3%    | 25.3%       | 11.4%     | 28.9%        |
